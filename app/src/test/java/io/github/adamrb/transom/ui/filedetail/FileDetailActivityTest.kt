...
class FileDetailActivityTest {
    ...
    fun setUp() {
        ...
        io.github.adamrb.transom.export.FileProviderTestSupport.resetCache()
        ...
        FileDetailActivity.playbackFactory = io.github.adamrb.transom.playback.Playback.Factory { fakePlayback }
    }

    /** A [io.github.adamrb.transom.playback.Playback] that records calls and lets a test move the playhead. */
    class FakePlayback : io.github.adamrb.transom.playback.Playback {
        val items = mutableListOf<io.github.adamrb.transom.playback.Playback.Item>()
        private val listeners = mutableListOf<io.github.adamrb.transom.playback.Playback.Listener>()
        ...
        var released = false
        override fun setItem(item: io.github.adamrb.transom.playback.Playback.Item) {
        ...
        override fun addListener(listener: io.github.adamrb.transom.playback.Playback.Listener) { listeners += listener }
        override fun removeListener(listener: io.github.adamrb.transom.playback.Playback.Listener) { listeners -= listener }
    ...
    fun tappingATimeChipSeeksThePlayerToTheParagraphsStart() {
        ...
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptWithParagraphs))
    ...
    fun playPauseAndSpeedChipsDriveThePlayer() {
        ...
        val next = launchServer(FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptJson)))
    ...
    fun jumpToListsBookmarksSpeakerChangesAndTimeMarks() {
        val fake = FakeServerSource(serverRecording().copy(durationS = 1500.0), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptWithParagraphs))
        val activity = launchServer(fake)
    ...
    fun theReadingPositionIsRememberedPerRecording() {
        ...
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(longTranscriptJson(300)))
        val controller = serverController(fake)
        shadowOf(android.os.Looper.getMainLooper()).idle()
    ...
    private class FakeServerSource(
        var rec: io.github.adamrb.transom.models.ServerRecording,
        var transcript: io.github.adamrb.transom.net.ApiClient.TranscriptResult
        ...
        var routing: io.github.adamrb.transom.net.ApiClient.RoutingResult =
            io.github.adamrb.transom.net.ApiClient.RoutingResult.Ok(emptyList())
        ...
        override suspend fun recording(id: String): io.github.adamrb.transom.net.ApiClient.RecordingResult {
            recordingCalls += id
            recordingGate?.await()
            if (recordingFailures > 0) {
                recordingFailures--
                return io.github.adamrb.transom.net.ApiClient.RecordingResult.Error("timeout")
            }
            return if (id == rec.id) io.github.adamrb.transom.net.ApiClient.RecordingResult.Ok(rec)
            else io.github.adamrb.transom.net.ApiClient.RecordingResult.NotFound
        }
        ...
        override suspend fun transcript(id: String): io.github.adamrb.transom.net.ApiClient.TranscriptResult {
            transcriptCalls += id
        ...
        override suspend fun routing(id: String): io.github.adamrb.transom.net.ApiClient.RoutingResult {
            routingCalls += id
            routingGate?.await()
            return routing
        }
        ...
        var rerunResult: io.github.adamrb.transom.net.ApiClient.ActionResult = io.github.adamrb.transom.net.ApiClient.ActionResult.Ok
        val rerunKeys = mutableListOf<String>()
        val rerunInstructions = mutableListOf<String?>()
        override suspend fun rerunRouting(id: String, idempotencyKey: String, instructions: String?): io.github.adamrb.transom.net.ApiClient.ActionResult {
            reruns += id
        ...
        var retryResult: io.github.adamrb.transom.net.ApiClient.RetryResult = io.github.adamrb.transom.net.ApiClient.RetryResult.Ok
        override suspend fun retryDelivery(deliveryId: String): io.github.adamrb.transom.net.ApiClient.RetryResult {
            retries += deliveryId
            return retryResult
        }
        val renames = mutableListOf<Pair<String, String>>()
        override suspend fun rename(id: String, title: String): io.github.adamrb.transom.net.ApiClient.RecordingResult {
            renames += id to title
            return io.github.adamrb.transom.net.ApiClient.RecordingResult.Ok(rec.copy(title = title))
        }
        ...
        var speakerRenameResult: io.github.adamrb.transom.net.ApiClient.TranscriptResult? = null
        override suspend fun renameSpeakers(id: String, renames: Map<String, String>): io.github.adamrb.transom.net.ApiClient.TranscriptResult {
            speakerRenames += renames
            speakerRenameResult?.let { return it }
            val ready = transcript as? io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready
                ?: return io.github.adamrb.transom.net.ApiClient.TranscriptResult.NotFound
            var json = ready.rawJson
            renames.forEach { (old, new) -> json = json.replace("\"$old\"", "\"$new\"") }
            return io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(json)
        }
        override suspend fun retranscribe(id: String) = io.github.adamrb.transom.net.ApiClient.ActionResult.Ok
        override suspend fun delete(id: String): io.github.adamrb.transom.net.ApiClient.ActionResult {
            deleted += id
            return io.github.adamrb.transom.net.ApiClient.ActionResult.Ok
        }
    }
    ...
    private fun noSpeechRecording(uploadedAt: String = "2026-09-01T05:30:00Z") = io.github.adamrb.transom.models.ServerRecording.fromJson(
        org.json.JSONObject(
            """{"id":"srv-9","device_sn":"SN-A","session_id":9,"filename":"9.mp3","size_bytes":1,
            "duration_s":61.0,"started_at":"2026-09-07T05:27:31Z","uploaded_at":"$uploadedAt",
    ...
    fun aLongTranscriptIsParsedOffTheMainThreadAndStillLandsWithItsParagraphs() {
        ...
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(json))
        val activity = launchServer(fake)
        shadowOf(android.os.Looper.getMainLooper()).idle()
        val rows = activity.transcriptRowsForTests()
        ...
        val short = launchServer(FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptWithParagraphs)))
    ...
    fun speakerRenameOnAnOlderServerSaysItNeedsAnUpdate() {
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptWithParagraphs))
        fake.speakerRenameResult = io.github.adamrb.transom.net.ApiClient.TranscriptResult.NotFound
        val activity = launchServer(fake)
        ...
        fake.speakerRenameResult = io.github.adamrb.transom.net.ApiClient.TranscriptResult.Error("HTTP 422", "New name must not be blank")
        activity.bindParagraphForTests(0).findViewById<View>(R.id.speakerLabel).performClick()
        latestDialog().apply {
            findViewById<android.widget.EditText>(R.id.textField)!!.setText("Alex")
            getButton(android.content.DialogInterface.BUTTON_POSITIVE).performClick()
    ...
    fun renameDialogUsesATextFieldAndRefusesABlankTitle() {
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptJson))
        val activity = launchServer(fake)
    ...
    fun serverModeShowsFetchedTitleAndTranscript() {
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptWithHighlights))
        val activity = launchServer(fake)
    ...
    fun theSummaryDropsItsHighlightsSectionWhenTheHighlightsRowsShowThem() {
        ...
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(withHighlightsSection))
        val activity = launchServer(fake)
        assertEquals("A greeting.", activity.findViewById<android.widget.TextView>(R.id.summaryText).text.toString())
        assertEquals(2, activity.findViewById<android.widget.LinearLayout>(R.id.highlightsList).childCount)
        ...
        val other = launchServer(FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(noRows)))
    ...
    fun theMoreButtonIsHiddenUntilThereIsARecordingToActOn() {
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
    ...
    fun summaryMarkdownIsRenderedNotShownRaw() {
        ...
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(markdownSummary))
        val activity = launchServer(fake)
    ...
    fun serverModePendingTranscriptShowsEmptyStateWithCheckButton() {
        val fake = FakeServerSource(serverRecording("transcribing"), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
        val activity = launchServer(fake)
        assertEquals("Server side \"Q3\" call", activity.findViewById<android.widget.TextView>(R.id.fileNameLabel).text.toString())
    ...
    fun noSpeechRecordingShowsOneLineKeepsThePlayerAndHidesTheTextActions() {
        val fake = FakeServerSource(noSpeechRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(noSpeechTranscript))
        val activity = launchServer(fake)
    ...
    fun aRecentNoSpeechUploadDoesNotWaitForARouterRun() {
        ...
        val fake = FakeServerSource(noSpeechRecording(uploadedAt = justNowIso()), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(noSpeechTranscript))
        val controller = serverController(fake)
        val activity = controller.get()
        val looper = shadowOf(android.os.Looper.getMainLooper())
    ...
    fun learningThatARecordingIsSilentEndsAWaitForItsRouterRun() {
        ...
        val fake = FakeServerSource(serverRecording("transcribing", uploadedAt = justNowIso()), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
        val activity = launchServer(fake)
        val looper = shadowOf(android.os.Looper.getMainLooper())
        fake.rec = noSpeechRecording(uploadedAt = justNowIso())
        fake.transcript = io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(noSpeechTranscript)
        val before = fake.routingCalls.size
    ...
    fun noSpeechRecordingBeingReTranscribedShowsTheProgressNotTheEmptyVerdict() {
        val file = storeFile(noSpeechTranscript, serverId = "srv-9")
        val fake = FakeServerSource(serverRecording("pending").copy(noSpeech = false, title = null), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
        configureServer(fake)
        val activity = launchBoth(file.id)
    ...
    fun transcribingStageAndPercentShowInTheBadgeAndTheBar() {
        val fake = FakeServerSource(
            ...
            io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending
        )
        val activity = launchServer(fake)
    ...
    fun transcriptionPollsEveryFewSecondsWhileWorkingThenLoadsTheTranscriptAndStops() {
        ...
        fake.transcript = io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptJson)
        looper.idleFor(FileDetailActivity.TRANSCRIPTION_POLL_INTERVAL_MS, java.util.concurrent.TimeUnit.MILLISECONDS)
    ...
    fun transcriptionPollingPausesWithTheScreenAndAsksAtOnceOnReturn() {
        val fake = FakeServerSource(serverRecording("pending"), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
        val controller = serverController(fake)
        val activity = controller.get()
        val looper = shadowOf(android.os.Looper.getMainLooper())
        assertEquals("Waiting to transcribe", activity.findViewById<android.widget.TextView>(R.id.statusBadge).text.toString())
    ...
    fun anAmbiguousFailureReplaysTheSameKeyAndTheSameInstructions() {
        ...
        fake.rerunResult = io.github.adamrb.transom.net.ApiClient.ActionResult.Error("timeout")
        val activity = launchServer(fake)
        val looper = shadowOf(android.os.Looper.getMainLooper())
        assertTrue(activity.onMenuAction(R.id.action_run_automations_with_instructions))
        latestDialog().apply {
        ...
        fake.rerunResult = io.github.adamrb.transom.net.ApiClient.ActionResult.Ok
        assertTrue(activity.onMenuAction(R.id.action_run_automations))
        looper.idle()
    ...
    fun intentForCarriesBothIdsWhenKnown() {
        ...
        val item = io.github.adamrb.transom.ui.recordings.RecordingItem(file, serverRecording())
        ...
        val serverOnly = FileDetailActivity.intentFor(context, io.github.adamrb.transom.ui.recordings.RecordingItem(null, serverRecording()))
    ...
    fun unifiedOpenKeepsThePhoneContentWhenTheServerIsUnreachable() {
        val file = storeFile(transcriptJson, serverId = "srv-9")
        val fake = object : FileDetailActivity.ServerDetailSource by FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending) {
            override suspend fun recording(id: String) = io.github.adamrb.transom.net.ApiClient.RecordingResult.Error("offline")
        }
        configureServer(fake)
        val activity = launchBoth(file.id)
    ...
    fun failedTranscriptionShowsFailedBadgeAndTheSingleCheckButton() {
        val fake = FakeServerSource(serverRecording("failed", error = "GPU on fire"), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
        val activity = launchServer(fake)
        val badge = activity.findViewById<android.widget.TextView>(R.id.statusBadge)
    ...
    fun aFailedRecordingShowsTheServersSentenceAndTheRawTextBehindDetails() {
        ...
        val plain = launchServer(FakeServerSource(serverRecording("failed", error = "Transcription failed."), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending))
    ...
    fun serverRefusalsShowTheServersOwnWordsNeverTheStatusCode() {
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptJson))
        fake.rerunResult = io.github.adamrb.transom.net.ApiClient.ActionResult.Error("HTTP 409", 409, "Automations are turned off on the server")
        val activity = launchServer(fake)
        assertTrue(activity.onMenuAction(R.id.action_run_automations))
        shadowOf(android.os.Looper.getMainLooper()).idle()
        ...
        fake.rerunResult = io.github.adamrb.transom.net.ApiClient.ActionResult.Error("HTTP 500", 500)
        assertTrue(activity.onMenuAction(R.id.action_run_automations))
        shadowOf(android.os.Looper.getMainLooper()).idle()
    ...
    private fun runsOf(json: String) = io.github.adamrb.transom.net.ApiClient.RoutingResult.Ok(
        io.github.adamrb.transom.models.RoutingRun.listFromJson(json)
    )

    private fun metaDate(iso: String): String =
        java.text.SimpleDateFormat("MMM d, yyyy \u00b7 HH:mm", java.util.Locale.getDefault())
            .format(java.util.Date(io.github.adamrb.transom.models.ServerRecording.parseIso(iso)!!))
    ...
    fun routingFetchFailureIsSilent() {
        val fake = FakeServerSource(serverRecording(), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptJson))
        fake.routing = io.github.adamrb.transom.net.ApiClient.RoutingResult.NotFound
        val activity = launchServer(fake)
        assertEquals(View.GONE, activity.findViewById<View>(R.id.automationsHeader).visibility)
        assertTrue(transcriptShown(activity))
    }
    ...
    fun recentlyUploadedRecordingWaitsForItsRouterRun() {
        ...
        val fake = FakeServerSource(serverRecording(uploadedAt = justNow), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Ready(transcriptJson))
        val activity = launchServer(fake)
        assertEquals(View.GONE, activity.findViewById<View>(R.id.automationsHeader).visibility)
        assertEquals(View.GONE, activity.findViewById<View>(R.id.automationsEmpty).visibility)
        val before = fake.routingCalls.size
    ...
    fun aFailedPollStillCountsAndTheNextOneRuns() {
        ...
        fake.routing = io.github.adamrb.transom.net.ApiClient.RoutingResult.Error("timeout")
        looper.idleFor(FileDetailActivity.ROUTING_POLL_DELAYS_MS[0], java.util.concurrent.TimeUnit.MILLISECONDS)
        assertEquals(before + 1, fake.routingCalls.size)
    ...
    fun waitingForARunWhosePollsAllFailStopsWithoutAVerdict() {
        val fake = FakeServerSource(serverRecording("transcribing"), io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending)
        val activity = openTranscribingThenTranscriptArrives(fake)
        fake.routing = io.github.adamrb.transom.net.ApiClient.RoutingResult.Error("offline")
        val before = fake.routingCalls.size
        val looper = shadowOf(android.os.Looper.getMainLooper())
        for (delay in FileDetailActivity.ROUTING_POLL_DELAYS_MS) {
            assertEquals(View.GONE, activity.findViewById<View>(R.id.automationsEmpty).visibility)
    ...
    fun runAutomationsHidesWhileTheServerIsReTranscribingEvenThoughTheOldTextIsStillShown() {
        ...
        fake.transcript = io.github.adamrb.transom.net.ApiClient.TranscriptResult.Pending
        assertTrue(activity.onMenuAction(R.id.action_retranscribe))
        shadowOf(android.os.Looper.getMainLooper()).idle()
        assertTrue(transcriptShown(activity))
        val menu = android.widget.PopupMenu(activity, activity.findViewById(R.id.moreButton)).also {
    ...
    fun aRetryThatFailsAmbiguouslyReReadsTheSection() {
        ...
        fake.retryResult = io.github.adamrb.transom.net.ApiClient.RetryResult.Error("timeout")
        val activity = launchServer(fake)
        val before = fake.routingCalls.size
