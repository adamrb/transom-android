...
class ApiClientTest {
    ...
    private val recordingJson = """{"id":"rec-1","device_sn":"SN1","session_id":42,"filename":"42.mp3",
        ...
        "source":"transom-android","status":"done","title":"Budget call","summary":null,"marks":[6.0],
    ...
    private val vocabularyJson = """{"entries":[
        ...
        "editor_text":"Morgan\nParrot Deck = Parted Deck, Carrot Deck","hotwords":"Parrot Deck, Morgan"}"""
    ...
    fun fetchVocabularyGetsEntriesAndEditorText() {
        ...
        assertEquals(
            listOf(
                io.github.adamrb.transom.models.VocabEntry("Parrot Deck", listOf("Parted Deck", "Carrot Deck"), "manual"),
                io.github.adamrb.transom.models.VocabEntry("Morgan", emptyList(), "obsidian")
            ),
            ok.entries
        )
        assertEquals("Morgan\nParrot Deck = Parted Deck, Carrot Deck", ok.editorText)
        val recorded = server.takeRequest()
        assertEquals("GET", recorded.method)
    ...
    fun saveVocabularyPutsEntriesAndRendersEditorTextLocally() {
        ...
        server.enqueue(
            MockResponse().setResponseCode(200).setBody(
                """{"entries":[{"term":"Parrot Deck","aliases":["Parted Deck"],"source":"manual"},
                {"term":"Morgan","aliases":[],"source":"obsidian"}]}"""
            )
        )
        val result = ApiClient.saveVocabulary(
            listOf(
                io.github.adamrb.transom.models.VocabEntry("Parrot Deck", listOf("Parted Deck"), "manual"),
                io.github.adamrb.transom.models.VocabEntry("Morgan", emptyList(), "obsidian")
            )
        )
        assertTrue(result.toString(), result is ApiClient.VocabularyResult.Ok)
        ...
        assertEquals("Morgan\nParrot Deck = Parted Deck", ok.editorText)
        val recorded = server.takeRequest()
    ...
    fun saveVocabulary422IsError() {
        ...
        val result = ApiClient.saveVocabulary(listOf(io.github.adamrb.transom.models.VocabEntry("x".repeat(80))))
