...
/**
 * PlaudBridgeJsInterface: the two hooks the dashboard may call. Work lands on the main looper,
...
class PlaudBridgeJsInterfaceTest {

    private lateinit var context: Context
    private val started = mutableListOf<Intent>()
    private var trusted = true

    private lateinit var bridge: PlaudBridgeJsInterface
    ...
    fun oversizedPayloadsAreRejected() {
        val big = "x".repeat(PlaudBridgeJsInterface.MAX_PAYLOAD_BYTES + 1)
        bridge.copyText(big)
        ...
        val multiByte = "会".repeat(PlaudBridgeJsInterface.MAX_PAYLOAD_BYTES / 3 + 1)
