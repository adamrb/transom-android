...
class DeviceManager private constructor() : DeviceManagerProtocol {
    ...
    /** Active Plaud user access token, fetched at runtime from transom-server. */
    private val partnerToken get() = io.github.adamrb.transom.net.TokenManager.currentToken
    ...
    override fun configure(userId: String) {
        ...
        val cached = io.github.adamrb.transom.net.TokenManager.cachedTokenIfValid()
        if (cached != null) {
        ...
        scope.launch(Dispatchers.IO) {
            try {
                val token = io.github.adamrb.transom.net.TokenManager.getValidToken()
    ...
    override fun connect(device: ScannedDevice, userId: String, userInitiated: Boolean) {
        ...
        scope.launch(Dispatchers.IO) {
            try {
                ...
                if (io.github.adamrb.transom.net.TokenManager.cachedTokenIfValid() == null &&
                    io.github.adamrb.transom.net.TokenManager.refreshAndApply() == null
                ) {
                    awaitingHandshake = false
    ...
    private fun cloudBind(sn: String) {
        scope.launch(Dispatchers.IO) {
            ...
            if (code == 401) {
                ...
                if (io.github.adamrb.transom.net.TokenManager.refreshAndApply() != null) {
