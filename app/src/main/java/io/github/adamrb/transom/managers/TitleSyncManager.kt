...
import io.github.adamrb.transom.work.TitleSyncScheduler
...
object TitleSyncManager {
    ...
    internal var onTranscriptStored: (file: RecordingFile, rawJson: String) -> Unit = { file, rawJson ->
        ...
        if (serverId != null) {
            ...
            val onScreen = try {
                io.github.adamrb.transom.ui.filedetail.FileDetailActivity.isShowing(serverId)
            } catch (t: Throwable) {
                false
            }
            if (context != null && !onScreen) {
                io.github.adamrb.transom.common.AppNotifications.transcriptReady(context, file.id, serverId, file.displayName, rawJson)
            }
            // Nothing to route when the server heard nothing: no router run is coming.
            if (!io.github.adamrb.transom.models.ServerRecording.transcriptSaysNoSpeech(rawJson)) {
