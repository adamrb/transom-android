...
import io.github.adamrb.transom.databinding.ActivityFileDetailBinding
import io.github.adamrb.transom.databinding.ViewFileDetailHeaderBinding
import io.github.adamrb.transom.export.ExportFileName
import io.github.adamrb.transom.export.TranscriptHighlight
import io.github.adamrb.transom.export.TranscriptMarkdown
import io.github.adamrb.transom.export.TranscriptParagraph
import io.github.adamrb.transom.export.TranscriptShare
import io.github.adamrb.transom.managers.TitleSyncManager
import io.github.adamrb.transom.models.Delivery
import io.github.adamrb.transom.models.RecordingFile
import io.github.adamrb.transom.models.RoutingRun
import io.github.adamrb.transom.models.ServerRecording
import io.github.adamrb.transom.net.ApiClient
import io.github.adamrb.transom.playback.ControllerPlayback
import io.github.adamrb.transom.playback.Playback
import io.github.adamrb.transom.playback.PlaybackService
import io.github.adamrb.transom.storage.RecordingStore
import io.github.adamrb.transom.ui.common.ContentWidth
import io.github.adamrb.transom.ui.common.MarkdownRenderer
import io.github.adamrb.transom.ui.common.themeColor
import io.github.adamrb.transom.ui.recordings.ApiServerRecordingActions
import io.github.adamrb.transom.ui.recordings.RecordingActions
import io.github.adamrb.transom.ui.recordings.RecordingItem
import io.github.adamrb.transom.ui.recordings.RecordingsAdapter
import io.github.adamrb.transom.ui.recordings.ServerRecordingActions
import java.io.File
...
class FileDetailActivity : AppCompatActivity(), JumpToSheet.Host, MoreActionsSheet.Host {
    ...
    private val syncManager get() = (application as TransomApp).syncManager
    ...
    companion object {
         ...
         */
        const val ACTION_NOW_PLAYING = "io.github.adamrb.transom.action.NOW_PLAYING"
    ...
    private fun backfillDuration(file: RecordingFile) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val d = io.github.adamrb.transom.managers.SyncManager.shared.audioDurationSec(localPath)
            if (d > 0) {
            ...
            when (val result = serverSource.retryDelivery(d.id)) {
                is ApiClient.RetryResult.Ok -> {
                    ...
                    serverRecordingId?.let { serverId ->
                        ...
                        io.github.adamrb.transom.managers.AutomationWatcher.watch(
    ...
    private fun runAutomations(serverId: String, instructions: String? = null) {
        ...
        rerunScope.launch {
            val result = try {
                ...
                io.github.adamrb.transom.managers.AutomationWatcher.unwatch(serverId)
                throw e
            }
    ...
    private fun watchAutomations(serverId: String) {
        ...
        val title = currentFile?.displayName ?: serverRecording?.displayTitle
        io.github.adamrb.transom.managers.AutomationWatcher.watch(serverId, currentFile?.id, title, knownDeliveries, knownRuns)
    }
    ...
    private fun watchAutomationsAfterTranscript(serverId: String) {
        val title = currentFile?.displayName ?: serverRecording?.displayTitle
        io.github.adamrb.transom.managers.AutomationWatcher.watch(serverId, currentFile?.id, title)
    }
    ...
    private fun bindAudio(file: RecordingFile?, rec: ServerRecording?) {
        ...
        val item = when {
            path != null && File(path).exists() -> {
                ...
                if (path.endsWith(".opus", ignoreCase = true)) io.github.adamrb.transom.common.OpusRepair.repairIfNeeded(path)
    ...
    private fun exportAudio(file: RecordingFile) {
        syncManager.exportAudio(file) { result ->
            runOnUiThread {
                ...
                result.onFailure {
                    io.github.adamrb.transom.common.AppLog.w("FileDetail", "audio export failed", it)
    ...
    private fun exportMarkdown(model: DetailModel) {
        ...
        try {
            ...
            io.github.adamrb.transom.common.AppLog.w("FileDetail", "markdown export failed", e)
            showAlert(getString(R.string.export_failed_title), getString(R.string.detail_export_failed_body))
        }
    }
