...
import cloud.adamrb.transom.databinding.ActivityFileDetailBinding
import cloud.adamrb.transom.databinding.ViewFileDetailHeaderBinding
import cloud.adamrb.transom.export.ExportFileName
...
import cloud.adamrb.transom.export.TranscriptShare
import cloud.adamrb.transom.managers.TitleSyncManager
...
import cloud.adamrb.transom.playback.ControllerPlayback
import cloud.adamrb.transom.playback.Playback
import cloud.adamrb.transom.playback.PlaybackService
import cloud.adamrb.transom.storage.RecordingStore
import cloud.adamrb.transom.ui.common.ContentWidth
import cloud.adamrb.transom.ui.common.MarkdownRenderer
import cloud.adamrb.transom.ui.common.themeColor
import cloud.adamrb.transom.ui.recordings.ApiServerRecordingActions
import cloud.adamrb.transom.ui.recordings.RecordingActions
import cloud.adamrb.transom.ui.recordings.RecordingItem
import cloud.adamrb.transom.ui.recordings.RecordingsAdapter
import cloud.adamrb.transom.ui.recordings.ServerRecordingActions
import java.io.File
...
class FileDetailActivity : AppCompatActivity(), JumpToSheet.Host, MoreActionsSheet.Host {
    ...
    companion object {
         ...
         */
        const val ACTION_NOW_PLAYING = "cloud.adamrb.transom.action.NOW_PLAYING"
    ...
    private fun backfillDuration(file: RecordingFile) {
        ...
        lifecycleScope.launch(Dispatchers.IO) {
            val d = cloud.adamrb.transom.managers.SyncManager.shared.audioDurationSec(localPath)
            if (d > 0) {
            ...
            when (val result = serverSource.retryDelivery(d.id)) {
                is ApiClient.RetryResult.Ok -> {
                    ...
                    serverRecordingId?.let { serverId ->
                        ...
                        cloud.adamrb.transom.managers.AutomationWatcher.watch(
    ...
    private fun runAutomations(serverId: String, instructions: String? = null) {
        ...
        rerunScope.launch {
            val result = try {
                ...
                cloud.adamrb.transom.managers.AutomationWatcher.unwatch(serverId)
                throw e
            }
    ...
    private fun watchAutomations(serverId: String) {
        ...
        val title = currentFile?.displayName ?: serverRecording?.displayTitle
        cloud.adamrb.transom.managers.AutomationWatcher.watch(serverId, currentFile?.id, title, knownDeliveries, knownRuns)
    }
    ...
    private fun watchAutomationsAfterTranscript(serverId: String) {
        val title = currentFile?.displayName ?: serverRecording?.displayTitle
        cloud.adamrb.transom.managers.AutomationWatcher.watch(serverId, currentFile?.id, title)
    }
    ...
    private fun bindAudio(file: RecordingFile?, rec: ServerRecording?) {
        ...
        val item = when {
            path != null && File(path).exists() -> {
                ...
                if (path.endsWith(".opus", ignoreCase = true)) cloud.adamrb.transom.common.OpusRepair.repairIfNeeded(path)
    ...
    private fun exportAudio(file: RecordingFile) {
        syncManager.exportAudio(file) { result ->
            runOnUiThread {
                ...
                result.onFailure {
                    cloud.adamrb.transom.common.AppLog.w("FileDetail", "audio export failed", it)
    ...
    private fun exportMarkdown(model: DetailModel) {
        ...
        try {
            ...
            cloud.adamrb.transom.common.AppLog.w("FileDetail", "markdown export failed", e)
            showAlert(getString(R.string.export_failed_title), getString(R.string.detail_export_failed_body))
        }
    }
